...
namespace QPil.Pages.Mensajes
{
    public class MessageBoxTemporal
    {
        ...
        MessageBoxTemporal(string texto, string titulo, int tiempo, bool contador)
        {
            ...
            if (contador)
            {
                System.Windows.Forms.DialogResult ResultadoMensaje = System.Windows.Forms.MessageBox.Show(texto + "\r\nEste mensaje se cerrará dentro de " +
                    TiempoMaximo.ToString("00") + " segundos ...", titulo);
                if (ResultadoMensaje == System.Windows.Forms.DialogResult.OK) IntervaloTiempo.Dispose();
            }
            else
            {
                System.Windows.Forms.DialogResult ResultadoMensaje = System.Windows.Forms.MessageBox.Show(texto + "...", titulo);
                if (ResultadoMensaje == System.Windows.Forms.DialogResult.OK) IntervaloTiempo.Dispose();
            }
        }
